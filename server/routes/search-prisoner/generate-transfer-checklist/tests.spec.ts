...
import { GenerateTransferChecklistSearchPrisonerPage } from './test.page'
...
test.describe('/search-prisoner/generate-transfer-checklist', () => {
  ...
  test('should try all cases', async ({ page }) => {
    await page.goto('/search-prisoner/generate-transfer-checklist')

    // verify page content
    const testPage = await new GenerateTransferChecklistSearchPrisonerPage(page).verifyContent()
    ...
    await expect(testPage.link('Create a transfer checklist for Name One')).toHaveAttribute(
      'href',
      /\/document-generation-ui\/download-document\/checklist-template-id\?prisonId=LEI&.+&prisonNumber=A1234EA/,
    )

    await testPage.verifyTableRow(2, ['Another Two', 'A2345EA', 'Loc-2'])
    await expect(testPage.link('Create a transfer checklist for Another Two')).toHaveAttribute(
      'href',
      /\/document-generation-ui\/download-document\/checklist-template-id\?prisonId=LEI&.+&prisonNumber=A2345EA/,
    )
  })
})
