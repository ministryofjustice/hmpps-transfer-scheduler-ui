...
import { PlanTransferCommentsPage } from './test.page'
...
test.describe('/plan-a-transfer/comments unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, '/plan-a-transfer/comments')
  })
})

test.describe('/plan-a-transfer/comments', () => {
  ...
  const startJourney = async (page: Page, journeyId: string) => {
    ...
    await page.goto(`/${journeyId}/plan-a-transfer/comments`)
  }

  test('should enter comments for plan-a-transfer', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)

    // verify page content
    const testPage = await new PlanTransferCommentsPage(page).verifyContent()
    ...
    expect(page.url()).toMatch(/\/plan-a-transfer\/check-answers/)
