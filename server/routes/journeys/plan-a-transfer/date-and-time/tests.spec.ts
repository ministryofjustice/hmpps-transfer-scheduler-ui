...
import { PlanTransferDateTimePage } from './test.page'
...
test.describe('/plan-a-transfer/date-and-time unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, '/plan-a-transfer/date-and-time')
  })
})

test.describe('/plan-a-transfer/date-and-time', () => {
  ...
  const startJourney = async (page: Page, journeyId: string) => {
    ...
    await page.goto(`/${journeyId}/plan-a-transfer/date-and-time`)
  }

  test('should enter date and time for plan-a-transfer', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)

    // verify page content
    const testPage = await new PlanTransferDateTimePage(page).verifyContent()
    ...
    expect(page.url()).toMatch(/\/plan-a-transfer\/destination/)
  ...
  test('should allow skip entering date and time for plan-a-transfer', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)
