...
import { PlanTransferLogisticsPage } from './test.page'
...
test.describe('/plan-a-transfer/logistics unauthorised', () => {
  test('should show unauthorised error', async ({ page }) => {
    await testNotAuthorisedPage(page, '/plan-a-transfer/logistics')
  })
})

test.describe('/plan-a-transfer/logistics', () => {
  ...
  const startJourney = async (page: Page, journeyId: string) => {
    ...
    await page.goto(`/${journeyId}/plan-a-transfer/logistics`)
  }

  test('should enter logistics for plan-a-transfer', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)

    // verify page content
    const testPage = await new PlanTransferLogisticsPage(page).verifyContent()
    ...
    expect(page.url()).toMatch(/\/plan-a-transfer\/comments/)
  ...
  test('should allow skip entering logistics for plan-a-transfer', async ({ page }) => {
    const journeyId = uuidV4()
    await startJourney(page, journeyId)
