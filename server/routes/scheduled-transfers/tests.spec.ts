...
import { BrowseScheduledTransfersPage } from './test.page'
...
test.describe('/scheduled-transfers', () => {
  ...
  test('should show search transfers page', async ({ page }) => {
    await page.goto('/scheduled-transfers?searchTerm=test&start=01/01/2001&end=31/01/2001&page=2')

    const testPage = await new BrowseScheduledTransfersPage(page).verifyContent()
    ...
    await expect(page.getByText('Enter a valid filter to search and view scheduled transfers.')).toBeVisible()
    ...
    await verifyAuditEvents([
      {
        ...
        details: expect.stringContaining(
          `"pageName":"BROWSE_SCHEDULED_TRANSFERS","query":"test","activeCaseLoadId":"LEI"`,
        ),
